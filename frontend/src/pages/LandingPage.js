...
export default function LandingPage({ onEnter, onRecruteur, onCandidat, onFonctionnalites }) {
  ...
  useEffect(() => {
    ...
    const waves = [
      { amp:60,  freq:0.008, speed:0.015, y:0.72, color:'rgba(79,70,229,0.22)'  },
      { amp:45,  freq:0.010, speed:0.020, y:0.78, color:'rgba(124,58,237,0.18)' },
      { amp:80,  freq:0.006, speed:0.010, y:0.68, color:'rgba(99,102,241,0.12)' },
      { amp:35,  freq:0.012, speed:0.025, y:0.82, color:'rgba(16,185,129,0.14)' },
      { amp:100, freq:0.005, speed:0.007, y:0.62, color:'rgba(79,70,229,0.07)'  },
      { amp:55,  freq:0.009, speed:0.018, y:0.12, color:'rgba(124,58,237,0.10)' },
      { amp:40,  freq:0.011, speed:0.022, y:0.06, color:'rgba(79,70,229,0.08)'  },
    ];
    ...
    const draw = () => {
      ...
      waves.forEach(w => {
        ...
        if (w.y > 0.5) { ctx.lineTo(canvas.width, canvas.height); ctx.lineTo(0, canvas.height); }
        else            { ctx.lineTo(canvas.width, 0);             ctx.lineTo(0, 0);             }
        ctx.closePath();
  ...
  return (
    <div style={{
      ...
      color:'#fff', fontFamily:"'Inter', sans-serif", overflowX:'hidden', position:'relative',
    }}>
      <style>{`
        ...
        .btn-cta {
          ...
          background-size:200% 100%; color:#fff; border:none; border-radius:14px;
          padding:17px 44px; font-size:16px; font-weight:700;
          cursor:pointer; transition:all .35s ease; box-shadow:0 0 0 0 rgba(79,70,229,.5);
        ...
        .btn-cta:hover { transform:translateY(-4px) scale(1.03); box-shadow:0 0 40px 8px rgba(79,70,229,.45); }

        .btn-outline {
          background:transparent; color:rgba(255,255,255,.8);
          border:1px solid rgba(255,255,255,.2); border-radius:14px;
          padding:17px 36px; font-size:16px; font-weight:500; cursor:pointer;
          transition:all .25s ease; backdrop-filter:blur(8px);
          display:inline-flex; align-items:center; gap:8;
        }
        .btn-outline:hover { border-color:rgba(79,70,229,.6); background:rgba(79,70,229,.12); color:#fff; transform:translateY(-3px); }
        ...
        .feature-card {
          ...
          border:1px solid rgba(255,255,255,.07); border-radius:20px; padding:32px 26px;
          transition:all .35s cubic-bezier(.34,1.2,.64,1); cursor:default; position:relative; overflow:hidden;
        }
        .feature-card::before {
        ...
        .stat-card {
          ...
          border-radius:16px; padding:24px 28px; text-align:center; transition:all .25s ease;
        }
        .stat-card:hover { background:rgba(79,70,229,.1); border-color:rgba(79,70,229,.3); transform:translateY(-4px); }
      `}</style>
      ...
      <div style={{ position:'fixed', inset:0, zIndex:0, pointerEvents:'none' }}>
        {[
          ...
          <div key={i} style={{
            ...
            filter:'blur(60px)', top:o.top, left:o.left, bottom:o.bottom, right:o.right,
            animation:o.anim,
          }} />
        ))}
      </div>
      ...
      <nav style={{
        ...
        background:'rgba(6,6,20,.7)', backdropFilter:'blur(28px) saturate(180%)',
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{
            ...
            fontSize:16, boxShadow:'0 0 20px rgba(79,70,229,.5)', animation:'glow 3s ease-in-out infinite',
          }}>💼</div>
        ...
        {/* Nav links — cliquables */}
        <div style={{ display:'flex', alignItems:'center', gap:32 }}>
          <button className="nav-link" onClick={onFonctionnalites}>Fonctionnalités</button>
          <button className="nav-link" onClick={onRecruteur}>Recruteurs</button>
          <button className="nav-link" onClick={onCandidat}>Candidats</button>
      ...
      <div style={{
        ...
        {[500, 700].map((size, i) => (
          <div key={i} style={{
            ...
            animation:`rotate ${(i+1)*20+10}s linear infinite ${i%2?'reverse':''}`,
            pointerEvents:'none',
          }}>
            <div style={{
              position:'absolute', top:i===0?-4:undefined, bottom:i===1?-4:undefined,
              left:'50%', width:i===0?8:6, height:i===0?8:6, borderRadius:'50%',
              background:i===0?'#4F46E5':'#7C3AED', transform:'translateX(-50%)',
              boxShadow:`0 0 ${i===0?12:10}px ${i===0?4:3}px ${i===0?'rgba(79,70,229,.8)':'rgba(124,58,237,.8)'}`,
            }} />
          </div>
        ))}
        ...
        <div style={{
          ...
          opacity:visible?1:0, animation:visible?'fadeUp .6s .1s ease both':'none',
        }}>
        ...
        <h1 style={{
          ...
          opacity:visible?1:0, animation:visible?'fadeUp .7s .2s ease both':'none',
        }}>
        ...
        <div style={{
          ...
          opacity:visible?1:0, animation:visible?'fadeUp .7s .3s ease both':'none',
        }}>nexttalent.ma</div>

        {/* Tagline */}
        <p style={{
          fontSize:'clamp(20px,3.5vw,30px)', fontWeight:300,
          color:'rgba(255,255,255,.85)', maxWidth:700, lineHeight:1.4, marginBottom:14,
          opacity:visible?1:0, animation:visible?'fadeUp .7s .4s ease both':'none',
        }}>
        ...
        <p style={{
          fontSize:15, color:'rgba(255,255,255,.35)', maxWidth:500, lineHeight:1.8, marginBottom:52,
          opacity:visible?1:0, animation:visible?'fadeUp .7s .5s ease both':'none',
        }}>
        ...
        <div style={{
          ...
          opacity:visible?1:0, animation:visible?'fadeUp .7s .6s ease both':'none',
        }}>
          <button className="btn-cta" onClick={onEnter}>🚀 Accéder à la plateforme</button>
          <button className="btn-outline" onClick={onEnter}>Créer un compte gratuit →</button>
        </div>
        ...
        <div style={{
          ...
          opacity:visible?1:0, animation:visible?'fadeUp .7s .8s ease both':'none',
        }}>
          {stats.map(s => (
      ...
      <div style={{ position:'relative', zIndex:1, padding:'60px 24px 100px', maxWidth:1100, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:52 }}>
          <h2 style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:'clamp(26px,4vw,40px)', fontWeight:700, color:'#fff', marginBottom:10 }}>
            Une plateforme, toutes les fonctionnalités
          </h2>
          <p style={{ color:'rgba(255,255,255,.3)', fontSize:15 }}>De l'import du CV jusqu'au choix final du candidat</p>
        </div>
        ...
        <div style={{ display:'flex', gap:12, justifyContent:'center', marginTop:40, flexWrap:'wrap' }}>
          {[
            { label:'🏢 Espace Recruteur', action: onRecruteur },
            { label:'👤 Espace Candidat',  action: onCandidat  },
            { label:'⚡ Toutes les fonctionnalités', action: onFonctionnalites },
          ].map(({ label, action }) => (
            <button key={label} onClick={action} style={{
              background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)',
              color:'rgba(255,255,255,.7)', borderRadius:10, padding:'10px 20px',
              fontSize:13, fontWeight:500, cursor:'pointer', transition:'all .2s',
            }}
              onMouseEnter={e => { e.currentTarget.style.background='rgba(79,70,229,.15)'; e.currentTarget.style.borderColor='rgba(79,70,229,.4)'; e.currentTarget.style.color='#fff'; }}
              onMouseLeave={e => { e.currentTarget.style.background='rgba(255,255,255,.05)'; e.currentTarget.style.borderColor='rgba(255,255,255,.1)'; e.currentTarget.style.color='rgba(255,255,255,.7)'; }}
            >{label}</button>
          ))}
        </div>
        ...
        <div style={{
          ...
          border:'1px solid rgba(79,70,229,.2)', borderRadius:24, padding:'52px 32px',
          position:'relative', overflow:'hidden',
        }}>
          <div style={{
      ...
      <div style={{
        position:'relative', zIndex:1,
        borderTop:'1px solid rgba(255,255,255,.05)', padding:'20px 48px',
        ...
        <span style={{ fontSize:11, color:'rgba(255,255,255,.15)' }}>LLaMA 3.1 · FastAPI · React.js · Railway · Vercel</span>
      </div>
    </div>
  );
}
